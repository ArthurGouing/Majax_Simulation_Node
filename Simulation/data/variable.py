# TODO create variable class, whith subclass for uniform var
# can be float, int, vector,
# Uniform or for all point // all groupe ??
# Variable uniform et non-unform hérite de Variable
import numpy as np


class Variable:
    def __init__(
        self,
        value=None,
        size: int = None,
        uniform: bool = False,
        stationary: bool = True,
        dtype: np.dtype = None,
    ) -> None:

        # Assert
        if size and value:
            print(
                "Error: have to choose between size or value to create OpenCL Buffer. If you specify value, you init the GPU buffer with the value. If you specify size, you will init an empty GPU buffer."
            )
            return 

        # Bool to tell if a Buffer has to be created on the GPU
        self.uniform: bool = uniform
        self.stationary: bool = stationary
        self.empty: bool = True if size else False

        # Set size
        if self.empty:
            self.size = size
            return

        # Set values
        # Convert to numpy, to be used by OpenCL
        if isinstance(value, float):
            self.value = np.float32(value)
        elif isinstance(value, int):
            self.value = np.int32(value)
        elif isinstance(value, list) or isinstance(value, tuple):
            self.value = np.array(value)
        else:
            self.value = value

        if dtype:
            self.value = self.value.astype(dtype)
